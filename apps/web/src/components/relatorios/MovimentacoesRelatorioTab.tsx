...
export function MovimentacoesRelatorioTab() {
  ...
  const {
    ...
    replacePath: "/relatorios?aba=movimentacoes",
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (sp.get("retornoOk") === "1") {
        ...
        const nextSp = new URLSearchParams(window.location.search);
        nextSp.delete("retornoOk");
        if (!nextSp.has("aba")) nextSp.set("aba", "movimentacoes");
        window.history.replaceState({}, "", `/relatorios?${nextSp.toString()}`);
      }
    }
    Promise.all([
  ...
  async function anexarTermo(id: string, file: File) {
    ...
    try {
      ...
      setMsg("Termo anexado e pendência resolvida.");
      fecharPainel();
      setReloadKey((k) => k + 1);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Falha no upload do termo");
    } finally {
  ...
  async function confirmarEstorno(id: string) {
    ...
    try {
      ...
      setMsg("Movimentação estornada com sucesso");
      fecharPainel();
      setReloadKey((k) => k + 1);
    } catch (e) {
  ...
  async function aprovar(id: string) {
    ...
    try {
      ...
      const extras = [
        res.alertaEstoqueMinimo ? "estoque mínimo" : "",
        res.alertaEstoqueMaximo ? "estoque máximo" : "",
      ]
        .filter(Boolean)
        .map((x) => `Alerta: ${x}`)
        .join(" · ");
      setMsg(
        res.mensagem
          ? `${res.mensagem}${extras ? ` · ${extras}` : ""}`
          : `Movimento aprovado${extras ? ` · ${extras}` : ""}`
      );
      setReloadKey((k) => k + 1);
    } catch (e) {
  ...
  function selecionarParceiro(c: Parceiro) {
    ...
    setParceiroTipoLabel(c.tipo === "FORNECEDOR" ? "Forn." : "Cliente");
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Entradas, saídas, transferências e rastreabilidade por número de série.
          </p>
        </div>
      ...
      <div className="mt-3 rounded-xl border bg-white px-3 py-2.5">
        <div className="grid grid-cols-2 items-center gap-2 sm:grid-cols-4 lg:grid-cols-12">
          ...
          <select
            ...
            className="col-span-1 min-w-0 rounded-md border border-slate-200 px-2 py-1.5 text-sm sm:col-span-1 lg:col-span-2"
            title="Filtrar por tipo de movimento"
            aria-label="Tipo de movimento"
          >
            <option value="">
              {operacaoFiltro ? "Tipo (todos da operação)" : "Tipo (todos)"}
            </option>
            {operacaoFiltro ? (
              tiposVisiveis.map((t) => (
                        ...
                        <td className="whitespace-nowrap px-3 py-2 text-right align-middle">
                          <div className="flex flex-nowrap items-center justify-end gap-2">
                            <Link
                              ...
                              title="Ver ficha completa do movimento"
                            >
                              Ver
                            </Link>
